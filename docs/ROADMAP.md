...
|------|-----------------|----|--------|------|
| 2026-05-11 | Phase 4 — regression tests + workflow + dashboard HTTP contracts | [#54](https://github.com/ishrith-gowda/overage/pull/54) | `8b0d9b2` | Squash-merged to `main`: unmocked `_record_and_estimate` alert path, `test_phase4_dashboard_api_contract.py`, `phase4-regression.yml`, `make test-phase4`, issue template; ROADMAP PR refs + closure table. |
...
**May 2026 follow-up (Phase 4.5)** — automatic **`DiscrepancyAlert`** inserts from the sliding window after estimation, pytest coverage for honoring % and the SDK client, and **`DISCREPANCY_ALERT_THRESHOLD_PCT`** — landed on **`main`** via **[PR #53](https://github.com/ishrith-gowda/overage/pull/53)** (squash `b0d4565`). Mock vs live limits are still documented in **§5 → Phase 0–4 — closure summary, mocks, and honest limits**.
...
**Are Phases 0–4 “all done”?** **Yes, for ledger / code / default CI:** every **Phase 0–4** row in §5 is marked **`done`**, `make check` (lint, mypy strict, `pytest proxy/tests/` excluding `slow`) is the local mirror of the main CI test gate, **Phase 3 PRD compliance** from **[PR #52](https://github.com/ishrith-gowda/overage/pull/52)** is on **`main`**, **Phase 4.5** (sliding-window alert auto-persist + tests + env) is on **`main`** via **[PR #53](https://github.com/ishrith-gowda/overage/pull/53)**, and **Phase 4 regression hardening** (unmocked alert pipeline test, dashboard JSON contracts, `phase4-regression` CI, `make test-phase4`) is on **`main`** via **[PR #54](https://github.com/ishrith-gowda/overage/pull/54)**. The table below still lists what is **mocked** or **not continuously proven** in CI.
...
| Phase 4.5 alert integration (fast gate) | **`test_record_and_estimate_persists_alert_when_window_sustained`** uses **patched** `aggregate_single_call` + patched `get_settings` + in-memory test DB; proves **DB insert + code path**, not vendor token drift. | Replace mocks / run load tests with **real** proxied traffic + **`ESTIMATION_ENABLED=true`** when validating in staging. |
| Phase 4 `_record_and_estimate` (May #54) | **`test_record_and_estimate_full_estimation_pipeline_inserts_alert`** runs **unmocked** `aggregate_single_call` with placeholder PALACE + timing regression + synthetic high-drift tokens (**[#54](https://github.com/ishrith-gowda/overage/pull/54)**); still **no** live OpenAI/Anthropic HTTP. | Use provider keys + staging for vendor-faithful drift. |
...
**PR refs.** [#18](https://github.com/ishrith-gowda/overage/pull/18) (merged 2026-04-07); **[#53](https://github.com/ishrith-gowda/overage/pull/53)** (merged 2026-05-11, squash `b0d4565`) — post-close sliding-window **auto-insert** of `DiscrepancyAlert`, honoring-rate pytest, SDK client smoke tests, env + ROADMAP alignment; **[#54](https://github.com/ishrith-gowda/overage/pull/54)** (merged 2026-05-11, squash `8b0d9b2`) — full estimation-path alert test (no aggregate mock), dashboard HTTP contract tests, `group_by` model/provider_model API tests, **`phase4_regression`** marker + workflow + **`make test-phase4`**.
...
- [x] **Post-audit (2026-05):** Phase 4.5 follow-up **[PR #53](https://github.com/ishrith-gowda/overage/pull/53)** squash-merged to `main` (`b0d4565`): wires `alert_persistence` into `_record_and_estimate`; adds `test_phase4_alert_persistence.py`, `test_sdk_client.py`, `TestSummaryHonoringRate`, **`DISCREPANCY_ALERT_THRESHOLD_PCT`**.
- [x] **Post-audit (2026-05):** Phase 4 regression **[PR #54](https://github.com/ishrith-gowda/overage/pull/54)** squash-merged to `main` (`8b0d9b2`): unmocked `_record_and_estimate` alert integration, **`test_phase4_dashboard_api_contract.py`**, `.github/workflows/phase4-regression.yml`, **`make test-phase4`**, issue template **`phase4_regression`**.
...
| Date | Event | Detail |
|------|-------|--------|
| 2026-05-11 | PR #54 on `main` + ROADMAP ledger | Code: squash **`8b0d9b2`** **[#54](https://github.com/ishrith-gowda/overage/pull/54)** — unmocked `_record_and_estimate` alert integration, **`test_phase4_dashboard_api_contract.py`**, `group_by` **model** / **provider_model** tests, **`phase4_regression`** marker + **`.github/workflows/phase4-regression.yml`** + **`make test-phase4`**, issue template, **`install_test_database_override`**. Ledger (this doc commit): **Recent landings** row; §5 “all done?” + **honest limits** table (patched fast gate vs full-path **#54** test); Phase 4 **PR refs** / DoD / post-close **Why**. |
